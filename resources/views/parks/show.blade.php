        ...
        <div class="container mx-auto px-4 py-4">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-4">
                    <a href="/" class="w-12 h-12 bg-[--color-primary-600] rounded-lg flex items-center justify-center hover:bg-[--color-primary-700] transition">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        @if($park->activations->count() > 0)
                            <div class="space-y-4">
                                ...
                                <div class="border-l-4 border-blue-500 pl-4 py-2 bg-gray-50 rounded-r">
                                    <div class="flex items-center justify-between">
                                        <div>
                                            <span class="font-mono font-bold text-lg text-gray-900">{{ $activation->callsign }}</span>
                                            <span class="text-sm text-gray-500 ml-2">{{ $activation->qso_count }} QSO</span>
                                        </div>
                                        <span class="text-sm text-gray-600">{{ $activation->activation_date->format('d.m.Y') }}</span>
                                    </div>
                                    @if($activation->notes)
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-white rounded-xl shadow-md p-6">
                        ...
                        <div class="space-y-3">
                            <button class="w-full px-4 py-3 bg-[--color-primary-600] text-white rounded-lg hover:bg-[--color-primary-700] transition font-semibold">
                            ...
                            <button class="w-full px-4 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 transition font-semibold">
                                ✓ Залогировать активацию
                            </button>
                            <button class="w-full px-4 py-3 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition font-semibold">
                    ...
                    <div class="bg-white rounded-xl shadow-md p-6">
                        ...
                        <div class="space-y-3">
                            <div class="flex justify-between items-center py-2 border-b">
                                ...
                                <span class="font-bold text-2xl text-[--color-primary-600]">{{ $park->activation_count }}</span>
                            </div>
                            <div class="flex justify-between items-center py-2 border-b">
                                <span class="text-gray-600">Уникальных позывных:</span>
                                <span class="font-bold text-2xl text-green-600">{{ $park->activations->unique('callsign')->count() }}</span>
                            </div>
                            <div class="flex justify-between items-center py-2">
                                <span class="text-gray-600">Всего QSO:</span>
                                <span class="font-bold text-2xl text-purple-600">{{ $park->activations->sum('qso_count') }}</span>
                            </div>
                        </div>
                    </div>
